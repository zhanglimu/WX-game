...
app.controller("aginforctrl", function($location, joinGameService) {
	...
	var urlObj = this;
	var promise = joinGameService.joinGame(); // 报名自由赛
...
app.factory('moneyService', ['$http', '$q', '$location', function($http, $q, $location) {
	return {
		queryMoney: function() {
				...
				$http({
					method: 'GET',
					url: url + 'agInfoMation/getFreeGameUserPosition.shtml?gp_id=' + gp_id + '&game_id=' + game_id + "&player_id=" + player_id
				}).
				success(function(data) {
					deferred.resolve(data); // 声明执行成功，即http请求数据成功，可以返回数据了  
				}).
...
app.factory('priceService', ['$http', '$q', '$location', function($http, $q, $location) {
	return {
		getAllPriHistory: function(opt) {
				...
				$http({
					method: 'GET',
					url: url + 'agInfoMation/getAllOddsHistory.shtml?game_id=' + game_id + "&opt=" + opt + "&player_id=" + player_id
				}).
				success(function(data) {
					deferred.resolve(data); // 声明执行成功，即http请求数据成功，可以返回数据了  
				}).
...
app.controller("modular", function($scope, $interval, oddsService, moneyService, queryTicketService, sellService, priceService, ticketService, newPriceService) { //比赛
	$scope.showProduct = "HAD";
	var mc = this;
	queryOdds($scope, oddsService, moneyService, queryTicketService, mc, priceService, "2");
	...
	$scope.tabtClick = function(ttype) {
			if(ttype == "HAD") {
				...
				queryOdds($scope, oddsService, moneyService, queryTicketService, mc, priceService, "1");
			} else {
				$scope.showProduct = "HAD";
				queryOdds($scope, oddsService, moneyService, queryTicketService, mc, priceService, "3");
			}
	...
	//日志
	$scope.pageNumClick = function(numtype) {
			if(numtype == '-1') { //上一页
				page_num = page_num - 1;
				queryOdds($scope, oddsService, moneyService, queryTicketService, mc, priceService, "2");
...
app.factory('ticketService', ['$http', '$q', '$location', function($http, $q, $location) {
	return {
		buyTicket: function() {
				...
				$http({
					method: 'GET',
					url: url + 'agInfoMation/buyFreeTicket.shtml?gp_id=' + gp_id + '&game_id=' + game_id + "&opt=" + $("#buyopt").val() + "&odds=" + $("#buyOdds").val() + "&money=" + $("#bet").val() + "&player_id=" + player_id
				}).
				success(function(data) {
					deferred.resolve(data); // 声明执行成功，即http请求数据成功，可以返回数据了  
				}).
...
app.factory('queryTicketService', ['$http', '$q', '$location', function($http, $q, $location) {
	return {
		queryTicket: function(type) {
				...
				$http({
					method: 'GET',
					url: url + 'agInfoMation/getAllOperLogs.shtml?game_id=' + game_id + "&page_num=" + page_num + "&type=" + type + "&game_type=1" + "&player_id=" + player_id
				}).
				success(function(data) {
					deferred.resolve(data); // 声明执行成功，即http请求数据成功，可以返回数据了  
				}).
...
function queryOdds($scope, oddsService, moneyService, queryTicketService, mc, priceService, opt1) {
	...
	promise.then(function(data) {
			var oddsObj = data;
			var tbody = new Array();
			var promiseMoney = moneyService.queryMoney(); //查询赔率  票
			promiseMoney.then(function(data) {
					if(data.data != null) {
					...
					//所有的交易记录
					var pages = 8;
					var promiseLog = queryTicketService.queryTicket(2); //查询日志
					promiseLog.then(function(data) {
						...
						if(data.data != null) {
							...
							for(var i = 0; i < total; i++) {
								var objnum = new Object();
								if(i + 1 == page_num) {
									$scope.currnummy = page_num;
								}
							}
							if(total > page_num) {
								$scope.xclass = true;
							...
							if(total > page_num && total-page_num>1) {
								$scope.currdmy = true;
							} else {
								$scope.currdmy = false;
							}
